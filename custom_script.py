...
class SDE_ReID(object):
    ...
    def postprocess(self, pred_dets, pred_embs):
        ...
        online_tlwhs, online_scores, online_ids, online_feats = [], [], [], []
    ...
    def postprocess_image(self, pred_dets, pred_embs):
        tracker = self.tracker
        tracker.predict()
        online_targets = tracker.update(pred_dets, pred_embs)
    ...
    def predict(self,
        ...
        # postprocess
        if pred_image:
            tracking_outs = self.postprocess_image(pred_dets, pred_embs)
        elif MTMCT == False:
    ...
    for i, img_file in enumerate(image_list):
        ...
        if len(pred_dets) == 1 and np.sum(pred_dets) == 0:
            ...
            if run_benchmark:
                ...
                tracking_outs = reid_model.predict(crops, pred_dets, pred_image=True)

                online_tlwhs = tracking_outs['online_tlwhs']
                online_scores = tracking_outs['online_scores']
                online_ids = tracking_outs['online_ids']
                online_feats = tracking_outs['online_feats']

                np.save("/home/aistudio/resources/ref_tlwh", np.asarray(online_tlwhs))
                np.save("/home/aistudio/resources/ref_feats", np.asarray(online_feats))
...
def predict_video(detector,
    ...
    save_base_path = "/home/aistudio/PaddleDetection/output_features"
    ...
    while (1):
        ...
        if len(pred_dets) == 1 and np.sum(pred_dets) == 0:
            ...
            for _tlwh, _id, _feat in zip(online_tlwhs, online_ids, online_feats):
                x1, y1, w, h = _tlwh
                intbox = tuple(map(int, (x1, y1, x1 + w, y1 + h)))
                tmp_save_path = os.path.join(save_base_path, str(_id))
                if not os.path.exists(tmp_save_path):
                    os.makedirs(tmp_save_path)

                time_str = str(time.time())
                crop_img = frame[intbox[1]: intbox[3], intbox[0]: intbox[2], :]
                tmp_img_path = os.path.join(tmp_save_path, time_str + ".jpg")
                cv2.imwrite(tmp_img_path, crop_img)

                tmp_npy_path =  os.path.join(tmp_save_path, time_str)
                np.save(tmp_npy_path, _feat)
            ...
            # print(len(crops), len(online_feats), len(online_ids))
            # exit()
            # if frame_id + 1 == 20:
            #     exit()
